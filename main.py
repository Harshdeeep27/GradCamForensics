...
class IID_Dataset(Dataset):
    def __init__(self, num, file, choice='train'):
        ...
        if num is None or num > len(self.filelist):
    ...
    def load_item(self, idx):
        ...
        return self.transform(img), self.tensor(mask[:, :, :1]), os.path.basename(fname1)
...
class Global_Local_Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        mask = torch.ones(tmp.size(0), tmp.size(2) * tmp.size(3), device=tmp.device)
        mask_index = (mask == 1).nonzero()[:, 1].view(tmp.size(0), -1)
        idx_b = torch.arange(tmp.size(0), device=tmp.device).long().unsqueeze(1).expand(tmp.size(0), mask_index.size(1))
        ...
        for t in range(1, dy_T):
            ...
            ind_1st_top = torch.zeros(tmp.size(0), tmp.size(2) * tmp.size(3), tmp.size(2) * tmp.size(3), device=tmp.device)
...
class IID_Model(nn.Module):
    def __init__(self):
        ...
        self.gen = self.networks.to(self.device)
...
class InpaintingForensics():
    def __init__(self):
        ...
        # E.g., file = [['C:\Harsh\gc_dresden_train\images\img1.png', 'C:\Harsh\gc_dresden_train\masks\mask1.png'],
        #              ['./...'],...]
        self.train_file = os.path.join(BASE_DIR, 'train_filelist.npy')
        self.val_file = os.path.join(BASE_DIR, 'val_filelist.npy')
        self.test_file = ''
